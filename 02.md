# Lesson2: Gemini CLIでバイブコーディングする準備を整える

Manusを利用して作成したアプリの雛形を、GitHub Codespaces上で改変するための準備を進めます。

ここまでのManusだけだと......
- フリープランでは利用できるトークン量に限りがある
- 他の手段との連携に限界がある
- iOSアプリのリリースなどは現状できない
- コードの中身を細かくいじることが出来ないので柔軟な機能追加がしにくい

などやりにくい点もあります。

ここからはより応用がしやすいように、AIコーディングエージェントに開発を引き継いで体験してもらいます。

OpenAI（ChatGPT）のCodex、Claude Code、GitHub Copilotなど色々なツールがありますが、今回は無料の範囲で検証しやすい`Gemini CLI`を利用して開発を進めます。
...
**この章はバイブコーディングの準備**なのでこの章を終えることで何か追加でアプリの見た目が変わるなどは無いのでご留意ください。アプリの見た目などは変わらないので少し地味です。

## 2. ハンズオン

ここからはコマンドの実行という手順が発生します。

コマンドの実行は1文字の誤りで動かないことも発生します。**書いてある手順を注意して読む**、**コピペできるところはコピペする**ということを再度意識しましょう

### 2-1. Manusで作成したアプリをGitHubに連携
...
`All repositories`を選んで`save`を押しましょう。

#### ② 過去にGitHubを利用しリポジトリがある方

予めリポジトリを作成している場合、`Only select repositories`を選択。Manusにアクセスされても問題ないリポジトリを選んで`save`を押しましょう。

> <img src="https://i.gyazo.com/f55f1a02c6c71a988a19cb87678f66fe.png" alt="gyazo.png" width="450px" />

何らかのリポジトリをManus側に共有しないと`save`ボタンが押せません
...
### 2-2. GitHub Codespacesの時間設定（スキップ可能 再掲）

進みが早い人はこちらの事前準備資料をみて時間の設定しましょう。

https://gist.github.com/n0bisuke/13a25255d4230ead770d73c47247a2fb

### 2-3. GitHub Codespacesの立ち上げ

ここから実際にバイブコーディングを進める環境の、GitHub Codespaceを立ち上げます。

作成された自身のリポジトリから、作業を再開します。
...
途中、次のような警告が出る場合は`今後表示しない`を押して非表示にしましょう。
...
#### 2-4-2. Codespacesの確認とターミナルの練習

- 立ち上げたCodespaceの状態を確認します
- ターミナル操作の練習をかねて、次のコマンドを**それぞれ**ターミナルに貼り付け。`Enterキー`を押して実行してみましょう

コピー&ペーストをする際に、初回はこのようなアラートが表示されますが`許可`を押してください。

> <img src="https://i.gyazo.com/591ecdcc083b7572e430c78c8b7f8721.png" width="450px" />
...
- `/home/codespace/nvm/current/bin/npm`と表示されれば成功
...
### 2-5. Gemini CLI のインストール

これから利用していくGemini CLIの準備を進めます

次のコマンドをコピー。ターミナルをクリックして貼り付けて`Enterキー`を押してください。

```sh
npm install -g @google/gemini-cli@latest
```

インストールで少し時間がかかります。以下の画像のように表示されれば成功しています。

> <img src="https://i.gyazo.com/8dfff8cf7b00f796a9a05f753cacbb7e.png" alt="gyazo.png" width="450px" />

### 2-6. Gemini CLI の初回実行とログイン

無事にインストールできたら、次はGeminiを使えるようにGoogleアカウントでログインします。

続けて次のコマンドをコピー。同じく、ターミナルに貼り付けて`Enterキー`で実行します。
...
- Gemini CLIが起動します。
- ターミナルを選択した状態で、`矢印キー`と`Enterキー`で進めることができます。
- `1. Yes`を選択（⚫︎がついている状態に）して`Enterキー`で確定
...
Geminiの利用は`APIキー`を利用することも可能ですが、今回は事前に準備していただいた`Googleのアカウントでログインする方法`で利用します。

`1. Login with Google`を選択して確定してすすみます。

> <img src="https://i.gyazo.com/837974b44f8e48c6048499c8fc750219.png" alt="gyazo.png" width="450px" />

認証を行います。赤枠で表示されているURLにアクセスします。
windowsの方は`ctrl`、macの方は`command`を押した状態でクリックしてください。

> <img src="https://i.gyazo.com/4d3326eae6130662e09501e4c0dd3a84.png" alt="gyazo.png" width="450px" />

リンクを開いて問題ないか確認されるので、`開く`を押してください
...
グレー枠の中にある認証コードをターミナルに入力します。`Copy`を押すとコピーできます

> <img src="https://i.gyazo.com/3af01339d624a5663652def5b8c47c1c.png" alt="gyazo.png" width="450px" />
 
ターミナルに戻り、コピーしたコードを貼り付けましょう。正しく貼り付けられると、以下の画像の赤枠のような状態になります。この状態で`Enterキー`を押します
...
ログインが成功したらAIに話しかけてみましょう。

`>` に続けて文字を入力することができます。`hello`のように、簡単な挨拶文を入力して`Enterキー`を押しましょう

> <img src="https://i.gyazo.com/bf71300e5ecc19f8f6148d71471efc80.png" alt="gyazo.png" width="450px" />
> 
> 必ずしも英語である必要はありません。

Geminiから応答が返ってきたら成功です。

### 2-8. Gemini CLI の停止　（スキップ可能）

**※進行が遅れている人はスキップしてください。**
...
- `/quit`が入力された状態で`Enterキー`を押すと、Gemini CLIが終了し、通常のターミナルに戻ります
...
- 通常のターミナルが開いている状態で再度`gemini`を入力して`Enterキー`を押すことでGemini CLIが起動します。
- 何回か停止と起動を繰り返して練習してみましょう。

### 2-9. ターミナルの分割

Gemini CLIの画面を使っていると、他にコマンドが実行しにくいです。
ターミナルを分割し、`Gemini CLI用`と`コマンド実行用`で分けましょう。

以下のGIF画像にある`分割ボタン`を押すことでターミナルを分割できます。

> <img src="https://i.gyazo.com/91050009d254c97b6bdfa120fab8da9b.gif" width="450px" />

分割した右側のターミナルはコマンド入力ができるようになります。

（[Tips](./03-tips.md)にShell Modeという説明がありますので、分かる方はそちらでもOKです）

### 2-10. 移植したアプリの確認　（スキップ可能）

Manusから移植したアプリの動作を確認します。 進行が遅れている人はスキップして大丈夫です。

Gemini CLIを開いて無い方のターミナルに次のコマンドをコピー・ペーストしたのち、`Enterキー`で実行してください。

```sh
npm run dev:metro
```

手順通りに来ていると、分割した右側のターミナルで進めていきます。

> <img src="https://i.gyazo.com/424bbd2b89b13724bd95f0182624629e.png" width="450px" />
...
> このとき表示される二次元バーコードは読み込んでも何も起きません
> 
> <img src="https://i.gyazo.com/7d00b8db66fa97af57f5641efad8dcca.png" alt="gyazo.png" width="450px" />
...
ブラウザーで該当URLを開くとManus上で表示されたものとほぼ同じアプリが表示されます。
開く際には少し時間が掛かります。
...
ここまでで確認は問題ないですが、時間がある人はせっかくですので、スマートフォンの幅で表示してみましょう。
...
### 2-11. ターミナルで実行しているプログラムの停止方法

前の章で`npm run dev:metro`などでプログラムを実行した際に、次の操作ができない状態になります。最後にこの実行しているプログラムの停止方法を覚えておきましょう。

ターミナルで実行しているプログラムは、キーボードの`ctrl` + `c`で停止できます。

右側のターミナルの赤枠のエリア、ターミナル上をクリックしてから、`ctrl` + `c`を押します。

> WindowsとMacではcontrolの役割が若干異なりますが、両OSともに`ctrl` + `c`で停止します  
> <img src="https://i.gyazo.com/27c45102ec30e54169ce00011ec36430.png" alt="gyazo.png" width="450px" />

新規にコマンドを受け付けられる状態になれば成功です。

> <img src="https://i.gyazo.com/ef2fc86581761575a8340b4bd9088445.png" alt="gyazo.png" width="450px" />

`$マーク`が表示されてることが目印です。

## 3. Tips

[こちらのTipsページ](./02-tips.md)に補足説明が書いてあります。
気になる人は見てみましょう。
...
- Tipsを見て、Gemini CLIのモードに何があるかを触ってみよう。
- Google検索などでGemini CLIでどんなことがやれるか調べてみよう。

## 5. この章のまとめ

この章では、`Manusで作成した雛形をGitHub Codespacesに移植`し、`Codespaces上でGemini CLIを利用してバイブコーディングするための準備`を行いました。Manusで作成したテンプレートを利用して、ここからはManusのトークン制限に縛られずに自由に開発ができます（Codexの従量課金は発生しますが）。
